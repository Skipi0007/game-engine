export const story = [
    {
        id: 0,
        text: "The story begins begins The story begins The story begins ",
        bg: require('./imges/textBg.jpg'),
        music: [require('./music/alert.mp3')]
    },
    {
        id: 1,
        text: "1st act story flow",
        bg: require('./imges/textBg2.jpg'),
        music: [require('./music/alert.mp3'), require('./music/alert.mp3')]
    },
    {
        id: 2,
        text: "2nd act",
        bg: require('./imges/textBg3.jpg'),
        music: [require('./music/alert.mp3')]
    },
    {
        id: 3,
        text: "The end",
        bg: require('./imges/textBg4.jpg'),
        music: [require('./music/icons.mp3')]
    },
    {
        id:4,
        text: ['That shit was werewrivere'],
        bg: require('./imges/textBg2.jpg'),
        choices: [{name: 'Dont answer', key: 0}, {name: 'Bratan prosnis, ti obosralsya', key: 1} ],
        music: [require('./music/icons.mp3')]
        
    }
    
]

